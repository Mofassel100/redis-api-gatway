...
import { corServiceUrl } from '../../../shared/axios';
const getAllFromDB = async (req: Request): Promise<IGenericResponse> => {
  const response: IGenericResponse = await corServiceUrl.get('/student-enrolled-course-marks', {
    params: req.query,
    headers: {
...
const getStudentMarks = async (req: Request): Promise<IGenericResponse> => {
  const response: IGenericResponse = await corServiceUrl.get(
...
const updateMarks = async (req: Request): Promise<IGenericResponse> => {
  const response: IGenericResponse = await corServiceUrl.post(
